...
has_tide_rider <- all(c("Latitude", "Longitude", "Depth") %in% names(d))
lat <- sites$latitude
lon <- sites$longitude
if(has_tide_rider) {
  lat <- mean(d$Latitude, na.rm = TRUE)
  lon <- mean(d$Longitude, na.rm = TRUE)
}
...
```{r tide rider, eval = has_tide_rider, child = "./tide_rider.Rmd"}
