...
sites_path <- params$paths$sites_path
metadata_path <- params$paths$metadata_path
preceding_data_path <- params$paths$preceding_data_path
paths <- params$paths
...
n_null <- sum(is.null(data_path), is.null(sites_path),
              is.null(metadata_path), is.null(preceding_data_path))
if (! n_null %in% c(0, 4))
...
has_preceding_data <- "preceding_data_path" %in% names(paths) &&
  !is.na(paths$preceding_data_path)
...
p <- bb_plot(d,
             ...
             range_limit = c(0, bb_options("max_raw_do"))
)
